...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='Project proposals with section/subsection structure and status field';
/*!40101 SET character_set_client = @saved_cs_client */;
...
--
-- Temporary view structure for view `proposal_vote_analysis`
--

DROP TABLE IF EXISTS `proposal_vote_analysis`;
/*!50001 DROP VIEW IF EXISTS `proposal_vote_analysis`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `proposal_vote_analysis` AS SELECT 
 1 AS `proposal_id`,
 1 AS `proposal_uuid`,
 1 AS `project_name`,
 1 AS `current_status`,
 ...
 1 AS `total_rejected`,
 1 AS `total_update`,
 1 AS `general_accepted`,
 1 AS `general_rejected`,
 1 AS `general_update`,
 1 AS `specific_accepted`,
 1 AS `specific_rejected`,
 1 AS `specific_update`*/;
SET character_set_client = @saved_cs_client;
...
INSERT INTO `role_submenu` VALUES (5,'211f472f-cde5-487e-b0ce-c7fef2a7a066',13,10,'2025-06-28 03:58:14','2025-06-28 03:58:14',NULL,NULL),(8,'9f773e6c-2114-40ce-9aed-8384d14285c5',11,12,'2025-07-17 00:50:29','2025-07-17 00:50:29',NULL,NULL),(9,'9a56f20c-527a-412e-97af-2a4ee35922b0',11,11,'2025-07-17 00:50:32','2025-07-17 00:50:32',NULL,NULL),(10,'37405286-f402-400a-9d0d-274766d1b8cc',10,12,'2025-07-17 18:08:23','2025-07-17 18:08:23',NULL,NULL),(11,'2556dec5-ff74-44d7-b11e-90bd11f26162',10,11,'2025-07-17 18:08:27','2025-07-17 18:08:27',NULL,NULL),(14,'f35f7a3e-8af1-46bd-aadf-db23dd83dd24',15,17,'2025-07-19 01:38:03','2025-07-19 01:38:03',NULL,NULL),(15,'6b79b00f-c3aa-426f-b261-eccbe6afa7a4',15,16,'2025-07-19 01:38:05','2025-07-19 01:38:05',NULL,NULL),(16,'f6245943-f0f3-479d-843e-c244596de96b',13,18,'2025-07-19 01:55:13','2025-07-19 01:55:13',NULL,NULL),(18,'d4e5f6a7-b8c9-0123-4567-890123defa04',14,19,'2025-09-27 04:57:44','2025-09-27 04:57:44',NULL,NULL),(19,'e5f6a7b8-c9d0-1234-5678-901234efab05',13,19,'2025-09-27 04:57:47','2025-09-27 04:57:47',NULL,NULL),(23,'f6a7b8c9-d0e1-2345-6789-012345678901',15,19,'2025-09-27 06:50:18','2025-09-27 06:50:18',NULL,NULL);
...
--
-- Final view structure for view `proposal_vote_analysis`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `proposal_vote_analysis` AS select `pp`.`id` AS `proposal_id`,`pp`.`uuid` AS `proposal_uuid`,`pp`.`project_name` AS `project_name`,`pp`.`proposal_status` AS `current_status`,count(`pc`.`id`) AS `total_votes`,sum((case when (`pc`.`vote_status` = 'ACEPTADO') then 1 else 0 end)) AS `total_accepted`,sum((case when (`pc`.`vote_status` = 'RECHAZADO') then 1 else 0 end)) AS `total_rejected`,sum((case when (`pc`.`vote_status` = 'ACTUALIZA') then 1 else 0 end)) AS `total_update`,sum((case when ((`pc`.`vote_status` = 'ACEPTADO') and (0 <> `is_general_vote`(`pc`.`section_name`,`pc`.`subsection_name`))) then 1 else 0 end)) AS `general_accepted`,sum((case when ((`pc`.`vote_status` = 'RECHAZADO') and (0 <> `is_general_vote`(`pc`.`section_name`,`pc`.`subsection_name`))) then 1 else 0 end)) AS `general_rejected`,sum((case when ((`pc`.`vote_status` = 'ACTUALIZA') and (0 <> `is_general_vote`(`pc`.`section_name`,`pc`.`subsection_name`))) then 1 else 0 end)) AS `general_update`,sum((case when ((`pc`.`vote_status` = 'ACEPTADO') and (0 = `is_general_vote`(`pc`.`section_name`,`pc`.`subsection_name`))) then 1 else 0 end)) AS `specific_accepted`,sum((case when ((`pc`.`vote_status` = 'RECHAZADO') and (0 = `is_general_vote`(`pc`.`section_name`,`pc`.`subsection_name`))) then 1 else 0 end)) AS `specific_rejected`,sum((case when ((`pc`.`vote_status` = 'ACTUALIZA') and (0 = `is_general_vote`(`pc`.`section_name`,`pc`.`subsection_name`))) then 1 else 0 end)) AS `specific_update` from (`project_proposals` `pp` left join `proposal_comments` `pc` on(((`pp`.`id` = `pc`.`proposal_id`) and (`pc`.`active` = 1)))) where (`pp`.`active` = 1) group by `pp`.`id`,`pp`.`uuid`,`pp`.`project_name`,`pp`.`proposal_status` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2025-10-01 11:23:09
